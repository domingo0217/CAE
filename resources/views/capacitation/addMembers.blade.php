    ...
    @section('content')
        <div class="card-panel white">
            ...
            <div class="col s12 m4">
                <form action="/searchCapacitationMember/{{ $capacitation->id }}" method="post">
                    {{ csrf_field() }}
            ...
            <form action="/capacitation_member/{{ $capacitation->id }}" method="post">
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                    @if(!empty($members[0]))
                    ...
                    <button type="submit" class="btn yellow darken-3 hoverable waves-effect right" name="button">Agregar</button>
            </form>
        </div>
    @endsection
